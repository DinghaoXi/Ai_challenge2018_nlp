...
# test_labels, test_ws = build_labels_ws('../data/',
#                                      'ai_challenger_sentiment_analysis_testa_20180816/sentiment_analysis_testa',
#                                      subjects_eng, cost_w)
...
def train_GCAE():
    ...
    test_aspect = np.tile(aspect_input, [len(test_context), 1, 1])
    ...
    test_aspect_ch = np.tile(aspect_input_ch, [len(test_context_ch), 1, 1, 1])

    train_aspect_input_ch_lens = np.tile(aspect_input_ch_lens, [len(train_context_ch), 1, 1])
    val_aspect_input_ch_lens = np.tile(aspect_input_ch_lens, [len(val_context_ch), 1, 1])
    test_aspect_input_ch_lens = np.tile(aspect_input_ch_lens, [len(test_context_ch), 1, 1])
    ...
    preds_1 = get_model_predict(model, score_detail,
                                [test_aspect, test_context, test_context_lens, train_ws[0:2000], test_feas,
                                 test_aspect_ch,
                                 test_context_ch, test_aspect_input_ch_lens, test_context_ch_len])
    df_ensemble_1, df_ensemble_test = ensemble_a_model(preds_1, score_detail, subjects_eng, factor=200, max_save_num=5,
                                                       ensemble_aspects=True)
    return df_ensemble_1


import sys

if __name__ == '__main__':
    df_ensemble_test = train_GCAE()
    print(df_ensemble_test)
    df_ensemble_test.to_csv('predict_GCAE.csv')